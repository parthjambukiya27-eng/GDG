...
const sendResetEmail = async (email, resetUrl, userName) => {
  ...
  if (useSMTP) {
    ...
    try {
      ...
      console.warn('Could not create Ethereal test account, email sending will be console-only:', err.message);
    }
  }

  const mailOptions = {
    from: process.env.SMTP_FROM || '"GDG IIT Bhilai Portal" <no-reply@iitbhilai.ac.in>',
    to: email,
    subject: 'GDG IIT Bhilai Portal - Password Reset Request',
    html: `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: auto; padding: 20px; border: 1px solid #e0e0e0; border-radius: 12px; background-color: #ffffff; color: #333333;">
        <div style="text-align: center; margin-bottom: 20px;">
          <h2 style="color: #4285F4; margin: 0;">Google Developer Groups</h2>
          <p style="font-size: 14px; color: #5f6368; margin: 2px 0 0 0;">IIT Bhilai Chapter</p>
        </div>
        <hr style="border: 0; border-top: 1px solid #eeeeee; margin: 20px 0;" />
        <p>Hello ${userName || 'Developer'},</p>
        <p>We received a request to reset your password for your GDG IIT Bhilai developer account.</p>
        <p>Please click the button below to reset your password. This link is valid for <strong>1 hour</strong>.</p>
        <div style="text-align: center; margin: 30px 0;">
          <a href="${resetUrl}" style="background-color: #4285f4; color: #ffffff; text-decoration: none; padding: 12px 24px; font-weight: bold; border-radius: 8px; display: inline-block; box-shadow: 0 4px 6px rgba(66, 133, 244, 0.22);">Reset Password</a>
        </div>
        <p style="font-size: 12px; color: #5f6368;">If the button above does not work, copy and paste this URL into your browser:</p>
        <p style="font-size: 12px; color: #4285f4; word-break: break-all;"><a href="${resetUrl}">${resetUrl}</a></p>
